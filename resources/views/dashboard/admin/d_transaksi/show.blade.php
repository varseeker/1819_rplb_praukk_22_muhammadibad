  ...
  <body>
    <nav class="navbar navbar-expand-sm navbar-dark bg-primary">
        ...
        <div class="collapse navbar-collapse" id="collapsibleNavId">
            <ul class="navbar-nav mr-auto mt-2 mt-lg-0">
                ...
                @if (Auth::user()->role == 'Admin' )
                  ...
                  <li class="nav-item active">
                      <a class="nav-link" href="{{ url('/dashboard/pesanan') }}">Data Pesanan <span class="sr-only">(current)</span></a>
                  </li>
                  <li class="nav-item">
                      <a class="nav-link" href="{{ url('/dashboard/outlet') }}">Data Outlet</a>
                  </li>
        ...
        <div class="container">
                <table class="table">
                        <tr>
                              <td><h1 class="user-select-none mt-4">Detail Data Pesanan</h1>
                                  <p class="user-select-none mb-2">Berikut adalah Detail data untuk Data Pesanan. </p>
                              </td>
                        </tr>
                </table>
                <div class="card">
                    <div class="card-header">
                        Dashboard > Data Pesanan > Detail
                        <form action="" method="POST" class="d-inline float-right">
                            @method('delete')
                            @csrf
                            <button type="submit" class="btn btn-danger">Delete</button>
                        </form>
                        <a name="" id="" class="btn btn-info float-right mr-3" href="../{{ $transaksi->id }}/edit" role="button">Update</a>
                    </div>
                    <div class="card-body">
                        ...
                        <p class="card-text">{{ $transaksi->nama }}</p>

                        {{-- Alamat --}}
                        <h5 class="card-title">Alamat</h5>
                        <p class="card-text">{{ $transaksi->alamat }}</p>
        
                        {{-- JENIS KELAMIN --}}
                        <h5 class="card-title">Jenis Kelamin</h5>
                        <p class="card-text">{{ $transaksi->jenis_kelamin }}</p>
        
        
                        {{-- NO TELEPHONE/HANDPHONE --}}
                        <h5 class="card-title">No Telephone</h5>
                        <p class="card-text">{{ $transaksi->tlp }}</p>
                    </div>
                </div>
            </div>
    <!-- Optional JavaScript -->
