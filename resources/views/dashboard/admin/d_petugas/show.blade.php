  ...
  <body>
    <nav class="navbar navbar-expand-sm navbar-dark bg-primary">
        ...
        <div class="container">
                <table class="table">
                        <tr>
                              <td><h1 class="user-select-none mt-4">Detail Data Petugas</h1>
                                  <p class="user-select-none mb-2">Berikut adalah Detail data untuk Data Petugas. </p>
                              </td>
                        </tr>
                </table>
                <div class="card">
                    <div class="card-header">
                        Dashboard > Data Petugas > Detail
                        <form action="" method="POST" class="d-inline float-right">
                            @method('delete')
                            @csrf
                            <button type="submit" class="btn btn-danger">Delete</button>
                        </form>
                        <a name="" id="" class="btn btn-info float-right mr-3" href="../{{ $petugas->id }}/edit" role="button">Update</a>
                    </div>
                    <div class="card-body">
        
                        {{-- ID --}}
                        <h5 class="card-title">ID</h5>
                        <p class="card-text">{{ $petugas->id }}</p>
        
                        {{-- NAMA --}}
                        <h5 class="card-title">Nama</h5>
                        <p class="card-text">{{ $petugas->nama }}</p>
                        ...
                        <p class="card-text">{{ $petugas->alamat }}</p>
        
                        {{-- JENIS KELAMIN --}}
                        <h5 class="card-title">Jenis Kelamin</h5>
                        <p class="card-text">{{ $petugas->jenis_kelamin }}</p>
        
        
                        {{-- NO TELEPHONE/HANDPHONE --}}
                        <h5 class="card-title">No Telephone</h5>
                        <p class="card-text">{{ $petugas->tlp }}</p>
                    </div>
                </div>
            </div>
    <!-- Optional JavaScript -->
